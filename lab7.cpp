...
int leap(int); //ok
int days(int); //ok
void getdate(void); //ok
int operator -(date); //ok
date operator +(int); //ok
friend ostream &operator<<(ostream &,date); //ok
friend int checkdate(date,date); //ok
};
...
switch(choice)
{
case 1: cout<<"Enter Date1 greater than Date2"<<endl;
d1.getdate();
d2.getdate();
i=checkdate(d1,d2);
if(i==1)
